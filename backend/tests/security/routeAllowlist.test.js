...
// auth exposure drifts from what the allowlist says -- fails this
...
// ADR 0005 removed authentication entirely, so there is no auth
// middleware left and every live route is public by construction.
// AUTH_MIDDLEWARE stays as the hook: if auth is ever reintroduced,
// point this at the middleware function and the exposure detection
// below starts working again with no other change.
const AUTH_MIDDLEWARE = null;
...
// with whether AUTH_MIDDLEWARE had been `router.use()`d before that route was
...
function enumerateRouterRoutes(prefix, router) {
  ...
  for (const layer of router.stack) {
    if (!layer.route) {
      // A `.use()` layer (e.g. an auth middleware, or
      // `router.use(express.json())`). Only AUTH_MIDDLEWARE, matched
      // by reference, changes exposure.
      if (AUTH_MIDDLEWARE && layer.handle === AUTH_MIDDLEWARE) {
        authApplied = true;
      }
      continue;
    }
...
describe("Route exposure allowlist (fitness function f3)", () => {
  ...
  it("declares an exposure for every mounted route, matching reality", () => {
    ...
    const actual = manifest.standalone.map((r) => ({
      ...
      // no auth concept; they are public by construction.
      exposure: "public",
    }));
    ...
    // actually does -- e.g. a `router.use(auth)` line got deleted
