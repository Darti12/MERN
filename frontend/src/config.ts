...
// changed how environment variables are exposed to the bundle
// (`process.env.REACT_APP_*` became `import.meta.env.VITE_*`), and this was
// the only file that had to change when it did.
//
...
if (!rawApiBaseUrl && import.meta.env.MODE !== "test") {
  ...
  console.warn(
    "VITE_API_URL is not set. The portfolio will render normally; the chat page will show as offline."
  );
}
